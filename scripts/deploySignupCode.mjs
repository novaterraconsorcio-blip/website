...
async function deploy() {
  const funcDir = path.resolve('supabase/functions/signup-code')
  const indexTs = path.join(funcDir, 'index.ts')
  if (!fs.existsSync(indexTs)) { console.error('index.ts não encontrado'); process.exit(1) }

  const metadata = { entrypoint_path: 'index.ts', name: 'signup-code', verify_jwt: false }
  const form = new FormData()
  form.append('metadata', new Blob([JSON.stringify(metadata)], { type: 'application/json' }), 'metadata.json')
  form.append('file', new Blob([fs.readFileSync(indexTs)], { type: 'application/typescript' }), 'index.ts')

  const res = await fetch(`https://api.supabase.com/v1/projects/${ref}/functions/deploy?slug=signup-code&verify_jwt=false`, {
    method: 'POST',
